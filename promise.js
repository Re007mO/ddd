...
function playGame(targetNum){
    if(rollDice()==targetNum){
        rollDice.then(function result(){
            console.log(result)
        })
    }
} */
...
const printStatusMessage = async (status) => {
  try{
    ...
    let message = `Animals listed as ${status} are: ${animalsList.join(', ')}`;
    console.log(message);
  }catch(error){
